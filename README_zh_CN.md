...
- CLI：适合脚本和自动化，常见入口包括 `alias`、`port`、`rules render`、`reload`、`config`、`doctor`、`config history` 和 `config rollback`；`--json`、稳定结果/错误分类，以及 `--check` / `--dry-run` 也更方便接入 Ansible 和 AI Agent。
